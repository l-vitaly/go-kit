...
func (s Subscriber) ServeMsg(nc *nats.EncodedConn) func(subject, reply string, d *Data) {
	return func(subject, reply string, d *Data) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()

		var m ptypes.DynamicAny
		if err := ptypes.UnmarshalAny(d.Data, &m); err != nil {
		...
		request, err := s.dec(ctx, m.Message)
		if err != nil {
			s.errorHandler.Handle(ctx, err)
			if reply == "" {
				return
			}
			s.errorEncoder(ctx, err, reply, nc)
			return
		}
		...
		if err := s.enc(ctx, reply, nc, response); err != nil {
			s.errorHandler.Handle(ctx, err)
			s.errorEncoder(ctx, err, reply, nc)
			return
		}
...
// types.
type ErrorEncoder func(ctx context.Context, err error, reply string, nc *nats.EncodedConn)
...
type SubscriberFinalizerFunc func(ctx context.Context, request interface{})
...
// a sensible default.
func EncodeJSONResponse(_ context.Context, reply string, nc *nats.EncodedConn, response interface{}) error {
...
// NopErrorEncoder ...
func NopErrorEncoder(_ context.Context, err error, reply string, nc *nats.EncodedConn) {
